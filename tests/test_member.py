...
async def test_member_join(bot):
    ...
    await testcord.member_join(name="Foo", discrim=5)
    ...
    await testcord.run_all_events()  # requires for the cov Greeting listner to be executed  # noqa: E501

    assert testcord.verify().message().content(f"Welcome {new_member.mention}.")
