...
async def test_add_role(bot):
    ...
    await testcord.add_role(member1, staff_role)
...
async def test_remove_role(bot):
    ...
    # then remove_role
    await testcord.remove_role(member1, staff_role)
...
async def test_remove_role2(bot):
    ...
    # First, we use add_role
    await testcord.add_role(0, staff_role)
    assert staff_role in guild.members[0].roles

    # then remove_role
    await testcord.remove_role(0, staff_role)
