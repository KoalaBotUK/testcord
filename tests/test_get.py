...
async def test_get_message_peek(bot):
    ...
    mess = testcord.get_message(peek=True)  # peek doesnt remove the message from the queue
...
async def test_get_embed_peek(bot):
    ...
    await channel.send(embed=embed)
    emb = testcord.get_embed(peek=True)
    assert emb.title == "Test Embed"
