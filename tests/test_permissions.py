...
async def test_permission_setting(bot):
    ...
    await testcord.set_permission_overrides(g.me, c, manage_roles=False)
    with pytest.raises(discord.errors.DiscordException):
        await c.set_permissions(m, send_messages=False, read_messages=True)

    await testcord.set_permission_overrides(g.me, c, manage_roles=True)
    ...
    await testcord.set_permission_overrides(g.me, c, manage_roles=False, administrator=True)
...
async def test_bot_send_not_allowed(bot):
    ...
    await testcord.set_permission_overrides(g.me, c, send_messages=False)
    with pytest.raises(discord.ext.commands.errors.CommandInvokeError):
        await testcord.message("!echo hello", channel=c)

    assert testcord.sent_queue.empty()

    perm = PermissionOverwrite(send_messages=True, read_messages=True)
    await testcord.set_permission_overrides(g.me, c, perm)
    await testcord.message("!echo hello", channel=c)
    assert testcord.verify().message().content("hello")
