...
async def test_member_join(bot):
    ...
    attach: discord.Attachment = discord.Attachment(
        state=testcord.back.get_state(),
        data=testcord.back.facts.make_attachment_dict(
            "test.jpg",
            15112122,
    ...
    message_dict = testcord.back.facts.make_message_dict(channel, author, attachments=[attach])
    try:
        message: discord.Message = discord.Message(state=testcord.back.get_state(), channel=channel, data=message_dict)
